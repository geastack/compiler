...
function eachChild(node: Nd, visitor: Visitor): Nd | undefined {
  const nodeVisitor: NodeVisitor = visitNode
  const plain = nodeVisitor(node.child, visitor)
  return plain ? nodeVisitor(plain, undefined, isLeaf) : nodeVisitor(node, visitor)
}
const v: Visitor = (n) => (n.kind === 7 ? { kind: 7 } : { kind: n.kind + 1 })
console.log(eachChild({ kind: 1, child: { kind: 7 } }, v)?.kind, eachChild({ kind: 1 }, v)?.kind)
//! expect: 7 2
