...
function verifyProperty(obj, name, desc, options) {
  assert.ok(arguments.length > 2, 'verifyProperty should receive at least 3 arguments: obj, name, and descriptor')
  ...
  assert.ok(__hasOwnProperty(obj, name), 'obj should have an own property ' + nameStr)
  ...
  if (failures.length) {
    assert.ok(false, __join(failures, '; '))
  }
...
function verifyWritable(obj, name, verifyProp, value) {
  if (!verifyProp) {
    assert.ok(__getOwnPropertyDescriptor(obj, name).writable, 'Expected obj[' + String(name) + '] to have writable:true.')
  }
...
function verifyNotWritable(obj, name, verifyProp, value) {
  if (!verifyProp) {
    assert.ok(!__getOwnPropertyDescriptor(obj, name).writable, 'Expected obj[' + String(name) + '] to have writable:false.')
  }
...
function verifyEnumerable(obj, name) {
  assert.ok(__getOwnPropertyDescriptor(obj, name).enumerable, 'Expected obj[' + String(name) + '] to have enumerable:true.')
...
function verifyNotEnumerable(obj, name) {
  assert.ok(!__getOwnPropertyDescriptor(obj, name).enumerable, 'Expected obj[' + String(name) + '] to have enumerable:false.')
  if (isEnumerable(obj, name)) {
...
function verifyConfigurable(obj, name) {
  assert.ok(__getOwnPropertyDescriptor(obj, name).configurable, 'Expected obj[' + String(name) + '] to have configurable:true.')
...
function verifyNotConfigurable(obj, name) {
  assert.ok(!__getOwnPropertyDescriptor(obj, name).configurable, 'Expected obj[' + String(name) + '] to have configurable:false.')
  if (isConfigurable(obj, name)) {
...
verifyProperty(Date.prototype.setFullYear, 'length', { value: 3, writable: false, enumerable: false, configurable: true })
console.log('setFullYear length ok')
